  ...
  <div class="inline-block group">
    ...
    >
      <template v-for="cate in categories">
        <li v-if="cate.children.length > 0" :key="cate.slug" class="relative p-3 rounded-sm hover:bg-gray-50">
          <button
          ...
          >
            <template v-for="child1 in cate.children">
              <li v-if="child1.children.length > 0" :key="child1.slug" class="relative p-3 rounded-sm hover:bg-gray-50">
                <button
                  class="flex items-center w-full text-left outline-none focus:outline-none"
                >
                  <span class="flex-1 pr-1">
                ...
                >
                  <template v-if="child1.children.length > 0">
                    <li v-for="child2 in child1.children" :key="child2.slug" class="p-3 hover:bg-gray-50">
              ...
              <li v-else :key="child1.slug" class="p-3 hover:bg-gray-50">
                <nuxt-link :to="`/danh-muc/${child1.slug}`">
                  {{ child1.name }}
                </nuxt-link>
              </li>
        ...
        <li v-else class="p-3 rounded-sm hover:bg-gray-50" :key="cate.slug">
          <nuxt-link :to="`/danh-muc/${cate.slug}`">
            {{ cate.name }}
          </nuxt-link>
        </li>
